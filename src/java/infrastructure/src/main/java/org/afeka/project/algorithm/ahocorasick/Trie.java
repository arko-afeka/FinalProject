...
public class Trie<V> {
  ...
  private void addStates(String keyword, V type) {
    ...
    for (Character c : keyword.toCharArray()) {
      currentState =
          currentState.getChildNodes().computeIfAbsent(c, (transition) -> new TrieNode<>());
    }
      ...
      currentNode
          ...
          .forEach(
              (transition, node) -> {
                ...
                while (!Objects.isNull(failureNode)
                    && failureNode.getNextNode(transition).isEmpty()) {
                  failureNode = failureNode.getFailureNode();
                }

                failureNode =
                    Objects.requireNonNullElse(failureNode, rootState)
                        .getNextNode(transition)
                        .orElse(rootState);
      ...
      for (int i = 0; i < tokens.size(); i++) {
        ...
        if (processed < currentTok.getBegin()) {
          tokens.add(
              i,
              new Token<>(
                  input.substring(processed, currentTok.getBegin()),
                  processed,
                  currentTok.getBegin(),
                  config.getDefaultType().get()));
        }
  ...
  private List<Token<V>> parseText(String input) {
    ...
    while (buffer.hasRemaining()) {
      ...
      currentState =
          currentState
              .getNextNode(nextChar)
              .orElse(currentState.getFailureNode().getNextNode(nextChar).orElse(this.rootState));
