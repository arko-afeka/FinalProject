...
public class Libinjection {
  ...
  private Keyword keywords; /* keyword hashmap */
  private State state;
  private String output;

  public Libinjection() {
    try (InputStream keywords = getClass().getClassLoader().getResourceAsStream("sqli/keywords.txt")) {
      this.keywords = new Keyword(keywords);
    } catch (Exception ex) {
      // Ignore
    }
  }
  ...
  public boolean is_keyword(String str) {
    return keywords.fingerprints.containsMatch(str);
  }
  ...
  public boolean libinjection_sqli_not_whitelist() {
    ...
    switch (tlen) {
      case 2:
      {
         ...
         * Note: evasion: 1*1--
         */
        if (state.tokenvec[0].type == TYPE_NUMBER && state.tokenvec[1].type == TYPE_COMMENT) {
          if (state.stats_tokens > 2) {
         ...
         */
        if ((state.tokenvec[1].len > 2) && state.tokenvec[1].val.charAt(0) == '-') {
          return false;
        }
      ...
      case 3:
      {
        ...
        if (fingerprint.equals("sos") || fingerprint.equals("s&s")) {
  ...
  public int libinjection_sqli_fold() {
    ...
    Token last_comment =
            new Token(CHAR_NULL, 0, 0, null); /* A comment token to add additional info */
    ...
    while (true) {
      ...
      if (pos >= LIBINJECTION_SQLI_MAX_TOKENS) {
        if ((state.tokenvec[0].type == TYPE_NUMBER
                ...
                && state.tokenvec[2].type == TYPE_LEFTPARENS
                && (state.tokenvec[3].type == TYPE_BAREWORD
                || state.tokenvec[3].type == TYPE_NUMBER)
                && state.tokenvec[4].type == TYPE_RIGHTPARENS)
      ...
      if (state.tokenvec[left].type == TYPE_STRING
        ...
        continue;
      } else if (syntax_merge_words(
              state.tokenvec[left], left, state.tokenvec[left + 1], left + 1)) {
        pos -= 1;
        state.stats_folds += 1;
        if (left > 0) {
          left -= 1;
  ...
  /**
   * Parsers: Looks at current character in input String, makes sense of it and turns it into a
   * token.
   */
  ...
  public int parse_string_core(char delim, int offset) {
    ...
    while (true) {
      if (qpos == -1) {
        ...
        Token token =
                new Token(TYPE_STRING, pos + offset, slen - pos - offset, s.substring(pos + offset));
        ...
        Token token =
                new Token(
                        TYPE_STRING, pos + offset, qpos - (pos + offset), s.substring(pos + offset, qpos));
        token.str_open = str_open;
    ...
    if (xlen == 0) {
      if (s.charAt(pos + 1) == '$') {
        ...
        if (strend == -1) {
          ...
          Token token =
                  new Token(TYPE_STRING, pos + 2, strend - (pos + 2), s.substring(pos + 2, strend));
          token.str_open = '$';
          token.str_close = '$';
          state.tokenvec[state.current] = token;
          return strend + 2;
        ...
        xlen =
                strlenspn(s.substring(pos + 1), "abcdefghjiklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
        if (xlen == 0) {
  ...
  public int parse_number() {
    ...
    if (have_e && !have_exp) {
      ...
      state.tokenvec[state.current] =
              new Token(TYPE_BAREWORD, start, pos - start, s.substring(start, pos));
    } else {
      state.tokenvec[state.current] =
              new Token(TYPE_NUMBER, start, pos - start, s.substring(start, pos));
    }
    return pos;
  }
